...
const lines = [
  ...
  '',
  '{',
  "  const mixedIndentUuid = '44444444-4444-4444-4444-444444444444';",
  '  const doc = [',
  "    '        - **链接格式转换**',",
  "    '          id:: ' + mixedIndentUuid,",
  "    '\\t\\t\\t- **Wikilinks to MDLinks**',",
  "    '\\t\\t\\t  - 功能：在 `[[Wikilinks]]` 和 Markdown links 之间互相转换。',",
  "    '- {{embed ((' + mixedIndentUuid + '))}}',",
  "  ].join('\\n');",
  "  const parsed = new BlockParser().parse('mixed-indent.md', doc);",
  "  const block = parsed.blocks.get(mixedIndentUuid);",
  "  assert.ok(block, 'mixed spaces/tabs source block should be indexed');",
  "  assert.match(block.childrenMarkdown, /Wikilinks to MDLinks/, 'tab-indented child should be included as childrenMarkdown');",
  "  assert.equal(parsed.referencesById.get(mixedIndentUuid)?.length ?? 0, 1, 'reference after mixed-indent source block should be found');",
  '}',
  '',
  '{',
